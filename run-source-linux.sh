...
# ═══════════════════════════════════════════════════════════════════════
# UAP Invaders - Linux Source Launcher
# ═══════════════════════════════════════════════════════════════════════
...
if ! command -v node &>/dev/null; then
    print_error "Node.js not installed. Install with: sudo apt install nodejs"
    exit 1
fi

if ! command -v npm &>/dev/null; then
    print_error "npm not installed. Install with: sudo apt install npm"
    exit 1
fi
...
fix_linux_sandbox() {
    if [ "$(sysctl -n kernel.unprivileged_userns_clone 2>/dev/null || echo 0)" -eq 0 ]; then
        print_warning "Electron sandbox not enabled. Run: sudo sysctl -w kernel.unprivileged_userns_clone=1"
    fi
}
